...
module.exports = {
    ...
    findOne: async (req, res) => {
        ...
        if (!project) return res.status(404).json({ message: { msgBody: "Project Not Found", msgError: true }});
    ...
    createProject: async (req, res) => {
        ...
        if (!user) return res.status(404).json({ message: { msgBody: "User not found", msgError: true }});
        else {
            ...
            newProject.save((err, project) => {
                if (err) return res.status(500).json({ message: { msgBody: "Error has occured", msgError: true }});
                else return res.status(200).json({ message: { msgBody: "Project successfully created.", msgError: false }});
            });
        }
    },
    findByUser: async (req, res) => {
       const projects = await db.Project.find({ userId: req.params.user_id});
       if (!projects) return res.status(404).json({ message: { msgBody: "No Projects Found", msgError: true }});
       else return res.status(200).json({ projects: projects, authenticated: true });
    },
    deleteProject: async (req, res) => {
        ...
        if (project.userId.toString() !== req.user.id.toString()) {
            ...
            project.remove(err => {
                if (err) return res.status(500).json({ message: { msgBody: "Error has occured", msgError: true }});
                else return res.status(200).json({message: { msgBody: "Project successfully deleted", msgError: false }});
            });
        }
    },
    addMember: async (req, res) => {
        ...
        if (!project) return res.status(404).json({ message: { msgBody: "No Project Found", msgError: true }});
            ...
            project.save(err => {
                if (err) return res.status(500).json({ message: { msgBody: "Error has occured", msgError: true }});
                else return res.status(200).json({ message: { msgBody: `${username} has been successfully added to ${project.title}.`, msgError: false }});
            })
        }
    }
}