...
module.exports = {
        ...
        db.User.findOne({ username }, (err, user) => {
            ...
            if (user) res.status(400).json({message: { msgBody: "Username is already taken", msgError: true }});
                ...
                newUser.save((err, savedUser) => {
                    if (err) return res.status(500).json({ message: { msgBody: "Error has occured", msgError: true }});
                    else return res.status(200).json({ message: { msgBody: "Account successfully created", msgError: false }});
                });
            }
        });
    },
    authenticateUser: (req, res) => { //confirms user is authenticated and sends user info to front end 
    ...
    login: (req, res) => { //login user route, assigns jwt token to user
    ...
    logout: (req, res) => { //logout user route, removes jwt token from user
